...
## F24. ホーム画面(PWA)から起動すると何も動かない — start_url がクエリを落とす
- 確認日: 2026-07-27(ユーザー実機)
- 事象: スマホでホーム画面にショートカットを作って起動すると、**セッションが表示されず何も動作しない**。
- 原因: `manifest.webmanifest` の `start_url` は `"./"`。**ホーム画面から起動するとペアリングのクエリ(`?room=&relay=&hpk=&ps=`)が丸ごと消える**。クライアントはこれを**URLからしか読んでおらず、保存していなかった**ため、
  - room が `default-room` にフォールバック
  - relay が `location.host`(= termhop.dev)にフォールバック
  - `hpk`/`ps` が無いので **E2EEではなく平文モード**
  となり、誰もいない別の部屋へ平文で繋ぎに行っていた。ブラウザのタブで開いている間は動くので気づきにくい。
- 対処: 受け取ったペアリング情報を `localStorage`(`ar_pairing_v1`)に保存し、**クエリが無いときは保存済みを使う**。あわせて、保存後に `history.replaceState` で**URLからペアリング秘密を消す**(アドレスバーと履歴に残さない。機能には影響しない)。
- 検証(Playwright): クエリ付きで開く→接続確立・URLから秘密が消える。その後**クエリ無しで開き直しても**「接続済み(E2EE)」・セッション表示・プリセット表示まで成立。
- 教訓: **PWAのホーム画面起動は「クエリ無しの初回訪問」と等価**。URLパラメータに依存する状態は必ず永続化する。ブラウザのタブでの検証だけでは発見できない。

## F25. スマホでのスクロールバックは経路を複数用意する
- 確認日: 2026-07-27
- F23 で touch ハンドラを入れたが、**実機ではまだ遡れなかった**。xterm 側がタッチイベントを先に処理して止めている可能性が高い。
- 対処(3経路。どれかが効けばよい):
  1. touch ハンドラを**捕捉フェーズ(`capture: true`)**に変更。xterm より先に受け取る。
  2. **掴めるスクロールバー**(`::-webkit-scrollbar` を 12px、`scrollbar-width:auto`)。既定の細いバーは指で掴めない。
  3. キーバーに **⤒ / ⤓(1画面ぶんスクロール)** を追加。これは端末への入力ではなく**表示のスクロール**で、`term.scrollLines(±(rows-1))` を呼ぶ。**タッチに依存しないので確実に動く**。
- ⚠️ **追記(2026-07-27、2巡目のFB)**: 上記で「遡れる」ようにはなったが、**実機で滑らかでなかった**。原因は `term.scrollLines()` が**行単位の離散移動**で、指の動きに追従しないこと。
  - 修正: `viewport.scrollTop` を**ピクセル単位で1:1追従**させ、離した後は**慣性(1フレームごとに0.94倍で減衰)**を付ける方式に作り替えた。ネイティブのスクロールと同等の手触りになる。
  - 検証(Playwright、TouchEvent合成): 150px ドラッグ → `scrollTop` が**ちょうど150px移動**(1:1)、離した後も慣性で188px継続。
  - 教訓: 「動く」と「使える」は別。**離散的な移動はスクロールの代用にならない**。位置追従+慣性まで作って初めてスクロールと呼べる。
- 教訓: **モバイルのジェスチャは環境差が大きい。**「効くはず」の実装に賭けず、**ボタンのような確実な代替経路**を必ず併設する。

## F26. 「ホストが居ない」状態が無言だった(保存済みペアリングの副作用)
- 確認日: 2026-07-27
- 背景: F24 でペアリングを localStorage に保存するようにした結果、**PC側の拡張が止まっている/再起動でペアリングが切れている**場合でも、クライアントはリレーに接続して**無言で待ち続ける**ようになった。ユーザーからは「繋がっているのにセッションが出ない」に見える。
- 実際に踏んだ経緯: 拡張を 0.3.0 に更新するために Antigravity を再起動 → **ホスト側のペアリングは失われる**(メモリ上のため) → スマホは保存済みペアリングで古い room に接続 → 無言。
- 対処: 接続後6秒たってもホストの応答(`kx.ack` / `host.hello`)が無ければ、**「PCが見つかりません。PC側で『ペアリング開始』を実行しQRを読み直してください」**と表示する。切断時はフラグを戻して再判定する。
- 検証: ホストを起動していない状態で開くと、6秒後に上記メッセージが出ることを確認。ホストがある場合は従来どおり「接続済み(E2EE)」。
- 教訓: **状態を永続化すると「古い状態で静かに失敗する」経路が生まれる。**保存を足したら、その状態が無効になったときの表示までセットで作る。

## F27. Antigravity の会話は IDE と CLI で保存先が別。両方とも本文を読み出せる
- 確認日: 2026-07-27
- **保存先が完全に別**なので、IDE の Agent Manager の会話は `agy`(Antigravity CLI v1.1.7)の履歴に出てこない。Antigravity 側の設計であり、こちらから統合はできない。
  - IDE: `~/.gemini/antigravity-ide/conversations/<id>.db`(6件)
  - CLI: `~/.gemini/antigravity-cli/conversations/<id>.db`(別ディレクトリ)
  - `agy --continue` / `--conversation <id>` は CLI 側のみを対象にする。
- **本文は読み出せる**(F2の続報)。`steps` テーブルの **`step_payload`** 列に入っている。protobuf だが、UTF-8 の可読文字列として抽出できる。
  - `step_type` 14=ユーザー発言 / 15=モデル出力 / 23=会話サマリ / 5,8,9=ツール呼び出し
  - ⚠️ 抽出の正規表現は**日本語(UTF-8マルチバイト)を必ず含める**こと。ASCII のみの正規表現で試すと「読めない」と誤判定する(最初これで詰まった)。
  - 実測例: `step_payload` から「確認項目: 地図の描画、現在地取得、看板画像のアップロード…」等が取れる。
...
## F29. 放置すると接続が切れる — 両側にキープアライブが無かった(実機FB)
- 確認日: 2026-07-27(ユーザー実機)
- 事象: スマホ側で一定時間操作しないと接続が切れる。
- 原因(複合):
  1. **両側ともキープアライブが無かった。** NAT・キャリア・中継は**無通信のTCPを黙って切る**ため、「繋がっているつもりで実は死んでいる」状態になり、操作しようとして初めて失敗する。
  2. **スマホはバックグラウンドでタイマーが凍結される。** 既存の再接続は `onclose` → `setTimeout(2s)` だけだったので、画面ロック中に切れると**復帰のタイマー自体が動かない**。
- 対処:
  - **クライアント**: 45秒ごとに `{t:"ping"}`(リレーは `t:"msg"` 以外を転送しないので**ホストには届かず中継の無駄がない**)。**画面が見えているときだけ**送る。100秒受信が無ければ死んだとみなし能動的に張り直す。`visibilitychange` / `pageshow` / `online` / `focus` で**前面復帰の瞬間に即再接続**(タイマーを待たない)。再接続は指数バックオフ(1→15秒)。
  - **ホスト**: 30秒ごとに WebSocket の **ping フレーム**。pong が90秒返らなければ `terminate()` して再接続に倒す。ping/pong は Cloudflare が Hibernation 中でも処理するため **DO を起こさない=課金対象にならない**(ブラウザは ping フレームを送れないのでアプリ層の ping を使っている)。
- ⚠️ **この修正で一度回帰を作った**: `focus`/`pageshow` の再接続が **CONNECTING(readyState 0)中にも発火**し、ソケットが二重になって `InvalidStateError: Still in CONNECTING state` が出た。
  - 対処: (a) 進行中判定に **CONNECTING も含める**、(b) `connect()` 内のハンドラは共有変数 `ws` ではなく**自分のソケット(`sock`)を見る**ようにし、差し替え後の古いハンドラは `stale()` で無視する。**再接続処理を書くときは「複数のソケットが同時に存在しうる」前提で書く。**
- 検証: `wrangler deploy` で**実際に全WebSocketを切断**して確認。ホストは自動で再接続しE2EEを再確立、クライアントも自動復帰して「接続済み(E2EE)」・セッション25件が戻った。

## F30. 一覧から消せない — 削除UIがそもそも無かった(実機FB)
- 確認日: 2026-07-28(ユーザー実機)
- 事象: 「過去に開いたセッションが一覧に残って『待機』と出る。手動で消せないか」
- 分かったこと2点:
  1. **「待機」= エージェント会話(🤖)**。機能B(F28)で `status:"idle"` を割り当てたため、24件がすべて「待機」で並んでいた。pty は running/waiting-input/exited しか取らないので、待機は必ず会話。
  2. **クライアントに削除UIが存在しなかった**。`session.close` はプロトコルにもホストにも実装済みなのに、**一覧から呼ぶ導線が無かった**ため、終了したターミナルすら消せなかった。
- 対処(クライアントのみ。ホストは変更不要):
  - 各行に **✕** を追加。
    - **会話**: ファイルなので**削除しない**。この端末に限って**隠す**だけ(`localStorage: ar_hidden_conv_v1`)。勝手に履歴を消すのは行き過ぎ。
    - **終了済みの端末**: そのまま `session.close`(消えるだけ)。
    - **動いている端末**: 誤タップ防止に**2回目のタップで確定**(1回目は「終了?」表示、3秒で戻る)。モーダルはスマホで重いので使わない。
  - **並び順を変更**: 端末(動いているもの)を先、会話を後ろ。同種内は新しい順。会話は件数が多く、混ぜると肝心の端末が埋もれる。
- 検証(Playwright): 会話を隠す 25→24 で localStorage に永続、実行中の端末は1回目で「終了?」のまま残り、2回目で 0 件になる、並び順が pty→agent。
- ⚠️ 検証時の落とし穴: **古いDOM参照を掴んだままだと2回目のタップが効かない**ように見えた。一覧はホストからの更新で再描画されるため、テストは毎回**要素を取り直す**必要がある(実際の指操作は常に画面上の現物を押すので問題にならない)。
- **追記(2026-07-28): 新しいQRでペアリングし直したら会話一覧をリセットする**(ユーザー要望)。
  - 理由: 過去の会話が残っても**そこから作業を再開できるわけではない**ので、一覧を占有するだけになる。
  - 実装: ペアリング時刻を `ar_paired_at_v1` に保存し、**それより前に更新が止まっている会話は畳む**。手動で隠した分(`ar_hidden_conv_v1`)もこの機に戻す。room が変わったときだけ実行するので、同じQRの読み直しや再接続ではリセットしない。
  - **完全には消さない**: 一覧末尾に「以前の会話 N件を表示」を出す。ここが無いとリセット後に過去の会話へ二度と辿り着けなくなる。展開状態は保存しないので、次に開くとまた畳まれる。
  - 副次的に良い性質: **ペアリング後に動きがあった会話は自動的に出てくる**(いま動いているエージェントが一覧に現れる)。検証でも、リセット直後は端末1件のみ→しばらく後に稼働中の会話2件が復帰し、残り22件は畳まれたままだった。

## F31. PCが落ちても無言だった — リレーの `peer-left` を捨てていた
- 確認日: 2026-07-28(ユーザー実機: 「✕では消せるのに、新規セッションを作っても無反応」)
- 事象と原因: **拡張を更新すると拡張ホストが再起動し、ペアリング(メモリ上)が失われる**。スマホは保存済みペアリングで古い room に繋ぎ続けるので、
  - ✕(削除)は**スマホ側で完結する操作**なので動く
  - ＋(新規作成)は**ホストの応答が要る**ので無反応
  という、切り分けが難しい症状になっていた。
- 根本原因: **リレーは `peer-left` / `peer-joined` を送っているのに、クライアントが `if (env.t !== "msg") return;` で捨てていた。** 相手の生死という重要な情報が手元にあったのに使っていなかった。
- 対処(クライアント):
  1. `peer-left`(role=host)を受けたら**即座に**「PCが切断されました。PC側で『ペアリング開始』を実行しQRを読み直してください」。従来は6秒タイマー頼りで、しかも接続済みのまま切れた場合は永久に無言だった。
  2. `peer-joined`(role=host)で張り直す(ホスト再起動後は鍵が変わるため、鍵交換をやり直す必要がある)。
  3. **ホスト不在のまま ＋ を押したら、押した瞬間に理由を出す**(送っても誰も応答しないため、以前は完全に無反応だった)。
  4. `kx.reject` の文言を「ペアリング拒否(秘密不一致)」から**次にとるべき行動**に変更。
- ⚠️ 検証で分かった正しい挙動: **ホストを再起動すると、同じ room でもペアリング秘密が作り直されるため古いQRは拒否される**(`kx.reject`)。これはセキュリティ上正しい。よって「拡張を更新したら必ずQRを読み直す」が正規の手順であり、UIはそれを明示すべきだった。
- 検証: ホストを kill → **即座に**「PCが切断されました…」を表示。ホスト不在で ＋ を押す → 押した瞬間に「PCが応答しません…」。同じ room でホストを立て直す → `kx.reject` を受けて「ペアリングが無効です…」。
- 教訓: **相手の生死を知らせる信号が既にあるなら、まずそれを使う。** タイムアウトでの推測は最後の手段。

## F32. 利用者数の計測(日替わりハッシュで突き合わせ不能にする)
- 確認日: 2026-07-28
- 目的: 告知前に「実利用者が何人いるか」を測れるようにする。従来の集計は接続本数・時間・メッセージ数だけで、**利用者数は分からなかった**。
- 設計(プライバシーを落とさずに数える):
  1. 拡張が**初回起動時に一度だけ**128bit乱数の `installId` を生成し `globalState` に保存。機器情報・アカウント・IPからは導出しない。
  2. リレー接続時の `hello`(**平文だが相手には転送されない層**)に載せる。E2EEの中身とは別経路。
  3. リレーは `SHA-256(installId + "|" + その日の日付)` の**先頭8バイトだけ**を保持。**生のIDは保存しない。**
  4. 日付がハッシュに入るので**値が毎日変わり、日をまたいだ同一性の突き合わせが運営者にもできない**。残るのは「その日いくつ異なる値が来たか」という整数だけ。
- 検証(本番): 同じIDを2回+別IDを1回送信 → `uniqueHosts` の増分は **2**(重複排除が効いている)。
- 上限 `MAX_HOSTS_TRACKED = 20,000` を置き、異常時にストレージが無制限に膨らまないようにした。
- ⚠️ **PRIVACY.md を同時に改訂した。**「個票を作らない」と約束している以上、集計対象を増やすなら**先に文書を直す**。`uniqueHosts` が何であり、なぜ日をまたいで追跡できないのかを明記した。
- 関連: README に**課金方針を先出し**(セルフホストは常に無料/今使えているものは取り上げない/課金対象は未実装の便利機能とマネージドリレーの上限)。後出しは信頼を最も損なうため。

## F33. フェアユースは「接続時間」ではなく「メッセージ量」で見る
- 確認日: 2026-07-29
- 発端: 「無制限だと重利用者に潰される」への対策として、ADR-007 の月50時間上限をどう実装するかを検討した。
- **測る対象が間違っていた。** Cloudflare の課金単位は DO リクエストで、**WSの受信メッセージ20通=1リクエスト**。Hibernation により**無通信の接続はDOが起きていない=ほぼ無料**。つまり:

  | 利用者像 | 接続時間 | メッセージ | 無料枠(10万req/日)の消費 |
  | --- | --- | --- | --- |
  | 放置(繋ぎっぱなし・無操作) | 24時間 | 2,000 | **0.1%** |
  | 普通の作業 | 3時間 | 50,000 | 2.5% |
  | 大量出力を流す | 1時間 | 3,000,000 | **150%** |

  → **接続時間はコストとほぼ無関係**。時間で上限を切ると、コストを生んでいない正規利用者を締め出し、本当に高い利用者を素通しすることになる。
- さらに、**時間課金は匿名性の放棄を伴う**。F32 で「日をまたいだ突き合わせが運営者にもできない」設計にした以上、月間利用時間は原理的に測れない。測るには恒久的な識別子(アカウント)が要り、PRIVACY.md の約束を撤回することになる。
- 決定: **room 単位・日次のメッセージ数上限(`ROOM_MSGS_PER_DAY = 300,000`)** を実装。
  - **匿名のまま**成立する(room はペアリングごとの乱数で、識別子ではない)。
  - ⚠️ **1メッセージ = 出力チャンク1つであり、「AIとのやり取り1回」ではない。** ここを取り違えると桁を1〜2つ間違える。
  - 実測(`SessionManager` に直接つないで計測):

    | 操作 | メッセージ数 |
    | --- | --- |
    | `ls -la` | 12 |
    | `git log --oneline -50` | 5 |
    | `npm run build` | 5 |
    | `seq 1 5000`(5000行) | 19 |
    | **`timeout 3 yes`(暴走)** | **32,968(= 11,000通/秒)** |

  - → 通常は1日200コマンド打っても**数千通**。一方で暴走は**秒あたり1万通**。両者は3桁違う。
  - 上限値の根拠: 30万通/日 = DOリクエスト15,000回 = **無料枠の15%**。冗長なビルドを1日10回まわす重い使い方(≒13万通)の2.3倍の余裕があり、暴走は**27秒で止まる**。
  - ⚠️ **当初 1,000,000 にしていたが緩すぎた**(1つの room が無料枠の50%を消費でき、暴走を91秒許す)。実測してから決め直した。
  - **AIとのやり取りに換算すると**(claude 対話TUIで実測): 放置20秒=**0通**(スピナーの再描画コストは無い)、短い1往復=**8通**、起動+放置+1往復=14通。
    → 30万通/日 ≒ **短い往復なら37,500回/日**、長文コード出力を伴う往復(推定50通)でも6,000回/日、大量ファイル出力を伴う重い往復(推定200通)でも1,500回/日。
    → 1日8時間ぶっ通しで1分に1回やり取りしても480往復/日。重く見積もって200通/往復でも**上限の32%**。**通常利用で到達することはない。**
  - 超過時は理由と代替(自前リレー)を伝えてから `close(1013)`。
  - ⚠️ 使用量は**永続化する**。メモリだけだと DO の退避でリセットされ上限が無意味になる(F18と同じ罠)。判定は100件ごと(1通ごとに書くと無料枠の書き込みを焼く)。
- 検証: `ROOM_MSGS_PER_DAY` を一時的に250へ下げて `relay-cf/scratch-cf-budget.js` → **close=1013 + 案内メッセージ**を確認。本番値に戻すと到達しない(正常)。既存の上限12項目・テレメトリ12項目も PASS。
- ⚠️ テストの教訓: テレメトリのテストが**累計値で判定していた**ため2回目以降必ず落ちた。**累積カウンタの検証は必ず差分で書く。**

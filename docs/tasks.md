...
### Stage 2: 収益化(**ADR-010 により保留**)
- **当面は課金しない**。TASK-28(決済)/TASK-29(時間の計測と執行)は**着手しない**。
  TASK-29 は濫用対策(room単位・日次30万通、F33)として実装済みの範囲で完了。
- 再検討の条件: (a)マルチホスト/チームの要望が実際に出る (b)原価が無料枠を継続的に超える (c)SLAを求められる。
- 支援口として GitHub Sponsors(`.github/FUNDING.yml`)。**GitHub側での有効化が必要**。
...
- **TASK-32 [P0] Windows / macOS 実機確認**(TASK-21の残): **Windows・Linux は確認済(2026-07-29、ユーザー)**。macOS のみ未確認だが、2OSで通っているため優先度は下がった。
...
### 進捗ログ(新しいものを上に)
- 2026-07-29 claude-opus: **ADR-010: 当面は課金しないと決定(ADR-007の時間課金を撤回)**。①時間はコストと無関係(F33) ②時間課金は匿名性の放棄を伴う(F32) ③原価が無料枠の3.8%で問題化していない。無料枠は「いま動いているものすべて・無制限」、濫用対策は課金と切り離す、収益化は**要望が出てから作る**。TASK-28/29は着手しない。GitHub Sponsors のみ用意(要GitHub側の有効化)。
- 2026-07-29 claude-opus: **フェアユース上限を実測で決め直した(F33追記)**。ユーザーの「100万通は多すぎでは」という指摘が正しかった。**1メッセージ=出力チャンク1つで、AIとのやり取り1回ではない**。実測すると通常操作は5〜19通/コマンドだが**暴走(`yes`)は11,000通/秒**。100万通は暴走91秒ぶんで、1つのroomが無料枠の半分を消費できてしまう緩さだった。→ **30万通/日**(無料枠の15%、重い使い方の2.3倍、暴走は27秒で停止)に変更。
- 2026-07-29 claude-opus: **フェアユース上限を実装(F33)**。「重利用者に潰される」対策だが、**測る対象が間違っていた**ことが判明。Cloudflareの課金はDOリクエストで、Hibernationにより**無通信の接続はほぼ無料**。24時間放置=0.1%、1時間の大量出力=150%。つまり接続時間はコストとほぼ無関係で、時間で切ると正規利用者を締め出し高コスト利用者を素通しする。→ **room単位・日次のメッセージ数上限(100万通/日)**に変更。**匿名のまま成立**し、PRIVACY.mdの約束を撤回せずに済む。**Windows/Linux の実機確認もユーザーが完了**(TASK-32)。
- 2026-07-28 claude-opus: **利用者数の計測を追加(F32)→ v0.5.0**。告知前に実利用者を測れるようにする。拡張が初回に乱数IDを1つ作り、リレーは**日付を混ぜてハッシュ化した値の個数だけ**を持つ(生IDは保存せず、日をまたいだ突き合わせが運営者にもできない)。本番で重複排除を確認(同ID×2+別ID×1→+2)。**PRIVACY.mdを同時改訂**(集計を増やすなら先に約束を直す)。あわせてREADMEに課金方針を先出し。**次**: Windows/macOS実機確認 → 告知(下書きは business/)。

 ...
 *
 * 値の根拠(すべて実測。F33):
 *   通常操作   : `ls -la`=12通 / `git log -50`=5通 / `npm run build`=5通 / `seq 1 5000`=19通
 *                → 1日200コマンド打っても数千通。**通常利用では当たらない**
 *   暴走(`yes`): **11,000通/秒**。3秒で33,000通
 *   冗長なビルドを1日10回まわす重い使い方でも 13万通程度
 *
 * 30万通/日 = DOリクエスト15,000回 = 無料枠の15%。重い使い方の2.3倍の余裕がありつつ、
 * 暴走は**27秒で止まる**。当初1,000,000にしていたが、それでは1つのroomが無料枠の
 * 半分を食い潰せてしまい、安全弁として緩すぎた。
 */
const ROOM_MSGS_PER_DAY = 300_000;
